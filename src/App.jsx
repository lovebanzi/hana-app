...
function Splash(){
  const fl=["🍼","🧸","⭐","🌙","💫","🎀","🐣","🌈","✨","🦋","💕","🌸"];
  return(
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"linear-gradient(160deg,#FFF0E6,#FFF8E6,#F0F8FF)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",animation:"splashOut 0.5s ease 2.8s forwards"}}>
      ...
      <div style={{textAlign:"center",zIndex:1,padding:"0 20px"}}>
        <div style={{fontSize:64,animation:"spinBounce 1.2s ease",marginBottom:12}}>☀️</div>
        <div style={{fontSize:40,fontWeight:900,letterSpacing:-2,background:`linear-gradient(135deg,${P},${G})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"fadeUp 0.6s 0.2s both"}}>HANA</div>
        <div style={{fontSize:14,color:TX,fontWeight:700,marginTop:8,lineHeight:1.7,animation:"fadeUp 0.6s 0.5s both",opacity:0}}>
          초보 엄마아빠를 위한<br/><span style={{color:P,fontWeight:900}}>궁금증 해소</span> 💕
        </div>
        <div style={{fontSize:11,color:MU,marginTop:8,animation:"fadeUp 0.6s 0.8s both",opacity:0}}>8개 쇼핑몰 통합 분석 · 월령별 맞춤 가이드</div>
      </div>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════
   ESSENTIALS SHEET
════════════════════════════════════════════════════ */
function EssSheet({month,onClose}){
    ...
    <div style={{position:"fixed",inset:0,zIndex:500,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:BG,borderRadius:"24px 24px 0 0",maxHeight:"88vh",display:"flex",flexDirection:"column",animation:"sheetUp .25s ease"}}>
        ...
        <div style={{padding:"6px 16px 12px",background:CA,borderRadius:"24px 24px 0 0",borderBottom:`1px solid ${BO}`}}>
          <div style={{fontSize:15,fontWeight:900,color:TX}}>📋 {month}개월 필수템 20가지</div>
          <div style={{marginTop:8,display:"flex",gap:5,flexWrap:"wrap"}}>
            {d.top5.map((item,i)=>(
              <span key={item} style={{background:i===0?`linear-gradient(135deg,${P},${G})`:`rgba(255,112,67,${0.12-i*0.02})`,color:i===0?"#fff":P,borderRadius:16,padding:"4px 10px",fontSize:11,fontWeight:800}}>
                {["🏆","🥈","🥉","✦","✦"][i]} {item}
              </span>
            ))}
          </div>
        ...
        <div style={{overflowY:"auto",flex:1,padding:"10px 14px 28px"}}>
          {d.items.map((item,i)=>(
            <div key={item.n} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 10px",borderRadius:12,marginBottom:5,background:i<5?"rgba(255,112,67,0.07)":CA,border:i<5?`1px solid rgba(255,112,67,0.2)`:`1px solid ${BO}`}}>
              <div style={{width:32,height:32,borderRadius:10,flexShrink:0,background:i<5?`linear-gradient(135deg,${P},${G})`:BG,display:"flex",alignItems:"center",justifyContent:"center",fontSize:i<5?15:11,fontWeight:900,color:i<5?"#fff":MU}}>{i<5?item.e:i+1}</div>
              <div style={{flex:1}}>
                <div style={{fontSize:12,fontWeight:800,color:i<5?P:TX}}>{i<5&&<span style={{fontSize:8,background:"rgba(255,112,67,0.15)",color:P,borderRadius:3,padding:"1px 4px",marginRight:3}}>필수</span>}{item.n}</div>
                <div style={{fontSize:10,color:MU}}>{item.r}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return(
    <div style={{
      ...
      <div style={{background:CA,padding:"11px 14px 9px",borderBottom:`1px solid ${BO}`,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:`0 2px 10px rgba(255,112,67,0.07)`}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:22,animation:"spin 8s linear infinite",display:"inline-block"}}>☀️</span>
          <div>
            <div style={{fontSize:19,fontWeight:900,letterSpacing:-1,lineHeight:1,background:`linear-gradient(135deg,${P},${G})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>HANA</div>
          ...
          return(
            <button key={id} onClick={()=>handleNav(id)} style={{flex:1,background:"none",border:"none",cursor:"pointer",padding:"8px 0 4px",display:"flex",flexDirection:"column",alignItems:"center",gap:2,position:"relative"}}>
              <div style={{position:"relative"}}>
                ...
                {badge>0&&<span style={{position:"absolute",top:-3,right:-5,background:`linear-gradient(135deg,${P},${G})`,color:"#fff",borderRadius:99,fontSize:7,fontWeight:900,padding:"1px 4px",minWidth:13,textAlign:"center"}}>{badge}</span>}
              </div>
              <span style={{fontSize:9,fontWeight:active?900:500,color:active?P:MU}}>{label}</span>
              {active&&<div style={{position:"absolute",bottom:0,width:20,height:2.5,borderRadius:3,background:`linear-gradient(135deg,${P},${G})`}}/>}
            </button>
          );
        })}
      </div>
      ...
      {showEss &&<EssSheet month={month} onClose={()=>setShowEss(false)}/>}
      ...
      {toast&&<div style={{position:"fixed",bottom:72,left:"50%",transform:"translateX(-50%)",background:`linear-gradient(135deg,${P},${G})`,color:"#fff",borderRadius:16,padding:"9px 18px",fontSize:12,fontWeight:800,zIndex:800,whiteSpace:"nowrap",pointerEvents:"none",animation:"toastIn .2s ease"}}>{toast}</div>}
